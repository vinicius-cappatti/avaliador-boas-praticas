...
#
# SÍNTESE DO ARQUIVO:
# Ponto de entrada principal do avaliador. Orquestra a leitura,
# validação PEP8 e a LLM.
#
...
import argparse
from utils import (
    ler_multiplos_arquivos,
    ler_arquivos_pasta,
    formatar_arquivos_com_pep8
)
...
def processar_github(
    url: str, limite_arquivos: int, limite_tamanho_kb: int
) -> dict[str, str]:
    """
    ...
    """
    return buscar_arquivos_codigo_github(
        url, limite_arquivos, limite_tamanho_kb
    )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Lê arquivos de código e armazena seus conteúdos em strings. "
            "Suporta arquivos locais ou repositórios GitHub."
        )
    )
    ...
    grupo_fonte.add_argument(
        ...
        type=str,
        help=(
            'Caminho para uma pasta local cujos arquivos de código serão '
            'analisados recursivamente.'
        )
    )
    ...
    if args.github:
        ...
        print(f"{'='*50}\n")
        try:
            conteudos = processar_github(
                args.github, args.limite_arquivos, args.limite_tamanho
            )
    ...
    conteudos = {
        nome: cont for nome, cont in conteudos.items() if nome.endswith(".py")
    }
    ...
    arquivos_formatados = formatar_arquivos_com_pep8(
        conteudos, resultados_pep8
    )

    # Monta o prompt completo
    prompt_final = PROMPT_CHECKLIST.format(
        arquivos_formatados=arquivos_formatados
    )
    ...
    try:
        ...
        with open("resposta_llama.md", "w", encoding="utf-8") as arquivo_saida:
            ...
            if args.github:
                ...
                fonte = args.diretorio
            else:
                fonte = ", ".join(args.arquivos)
