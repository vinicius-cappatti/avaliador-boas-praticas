...
#
# SÍNTESE DO ARQUIVO:
# Módulo para integração com GitHub API para buscar arquivos de
...
def listar_arquivos_repositorio(
    owner: str, repo: str, branch: str, caminho: str = ""
) -> list[dict]:
    """
    ...
    for item in itens:
        ...
        if tipo == "dir":
            ...
            arquivos.extend(
                listar_arquivos_repositorio(owner, repo, branch, caminho_item)
            )
        elif tipo == "file":
            arquivos.append({
                "nome": nome,
...
def buscar_arquivos_codigo_github(
    url_repositorio: str,
    limite_arquivos: int = 20,
    limite_tamanho_kb: int = 100
) -> dict[str, str]:
    """
    Função principal que busca e retorna arquivos de código de um
    repositório GitHub.
    ...
    Returns:
        Dicionário com nome do arquivo (caminho) como chave e
        conteúdo como valor.
    """
    ...
    arquivos_codigo = [
        a for a in arquivos_codigo if a.get("tamanho", 0) <= limite_bytes
    ]
